...
var vi = (function() {
    ...
    function editor_disable(sav) {
	    ...
	    document.body.style.overflow = '';
			...
			// Grab text and send to invoker
			var text = $(term).children("pre").filter(function(line) { return $(this).text().match(/^(.?$|[^~].*)/); }).text();
			document.viInvoker.closeVI(text);
    }
    ...
    return function(invoker, textarea, options) {
			document.viInvoker = invoker;		
	    ...
	    */
	    ...
	    cursor.ondblclick = _pass_dblclick;
			...
			// MY VERSION 
	    $(textarea).after(suggest);
	    $(textarea).after(term);
	    // firefox bug
	    if (once) $(textarea).after(backing);
	    $(textarea).parent().append(cursor);

	    cursoriv = window.setInterval(_redraw_cursor, 600);
